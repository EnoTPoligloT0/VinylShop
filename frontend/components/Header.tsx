...
function Header() {
    ...
    useEffect(() => {
        const newTotalPrice = cart.reduce((acc, item) => {
            ...
            const quantity = item.quantity || 0; // Fallback to 0
            return acc + (price * quantity);
        }, 0);

        const newItemCount = cart.reduce((acc, item) => acc + (item.quantity || 0), 0);
            ...
            <div className="container mx-auto grid grid-cols-12 items-center py-4">
                ...
                <div className="col-start-11 col-span-2 flex justify-end items-center space-x-4">
                    ...
                    <div className="relative flex items-center">
                        ...
                        <p className="ml-2 hidden md:block">${totalPrice.toFixed(2)}</p>
                    </div>
                </div>
            </div>
